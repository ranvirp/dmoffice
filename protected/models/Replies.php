...
class Replies extends CActiveRecord
{
	...
	protected function beforeSave()
	{
      ...
      public static function getReplies($content_type,$content_type_id)
      {
          try
          {
              $models=Replies::model()->findAllByAttributes(array('content_type'=>$content_type,'content_type_id'=>$content_type_id),array('order'=>'create_time DESC'));
...
return null;
      ...
      public static function lastReply($content_type,$content_type_id)
      {
                    try {
         $model=Replies::model()->findByAttributes(array('content_type'=>$content_type,'content_type_id'=>$content_type_id),array('order'=>'create_time DESC'));
          if ($model)
              return $model;
          else return null;
          } catch (Exception $ex) {
